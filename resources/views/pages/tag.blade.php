    ...
    <main id="main-contents" class="col-xs-12 col-sm-12 col-md-8">
        ...
        <section>
            <div class="section-bar clearfix">
                <h1 class="section-title"><span>Từ khóa: {{$tag}}</span></h1>
            </div>
            <div class="halim_box">
                @foreach($movie as $key => $mov)
                <article class="col-md-3 col-sm-3 col-xs-6 thumb grid-item post-37606">
                <div class="halim-item">
                    <a class="halim-thumb" href="{{route('movie', Str::slug($mov->slug))}}">
                        <figure><img class="lazy img-responsive" src="{{asset('uploads/movie/'.$mov->image)}}" alt="" title="{{$mov->title}}"></figure>
                        <span class="status">{{$mov->qualiti}}</span><span class="episode"><i class="fa fa-play" aria-hidden="true"></i>{{$mov->subtitle}}</span> 
        ...
        <div id="halim_tab_popular_videos-widget-7" class="widget halim_tab_popular_videos-widget">
            ...
            <section class="tab-content">
                ...
                <div class="popular-post">
                    <div id="view-by-day">
                        @foreach ($view_day as $key => $mov_view)
                        <div class="item post-37176">  
                                <a href="" title="{{$mov_view->movie->title}}">
                                    <div class="item-link">
                                    <img src="{{asset('uploads/movie/'.$mov_view->movie->image)}}" class="lazy post-thumb" alt="{{$mov_view->movie->title}}" title="{{$mov_view->movie->title}}" />
                                    <span class="is_trailer">{{$mov_view->movie->qualiti}}</span>
                                    </div>
                    ...
                    <div id="view-by-month">
                        @foreach ($view_month_total as $key => $mov_view)
                        <div class="item post-37176">  
                                <a href="" title="{{$mov_view->movie->title}}">
                                    <div class="item-link">
                                    <img src="{{asset('uploads/movie/'.$mov_view->movie->image)}}" class="lazy post-thumb" alt="{{$mov_view->movie->title}}" title="{{$mov_view->movie->title}}" />
                    ...
                    <div id="view-by-year">
                        @foreach ($view_year_total as $key => $mov_view)
                        <div class="item post-37176">  
                                <a href="" title="{{$mov_view->movie->title}}">
                                    <div class="item-link">
                                    <img src="{{asset('uploads/movie/'.$mov_view->movie->image)}}" class="lazy post-thumb" alt="{{$mov_view->movie->title}}" title="{{$mov_view->movie->title}}" />
                    ...
                    <div id="view-all">
                        @foreach ($view_all_total as $key => $mov_view)
                        <div class="item post-37176">  
                                <a href="" title="{{$mov_view->movie->title}}">
                                    <div class="item-link">
                                    <img src="{{asset('uploads/movie/'.$mov_view->movie->image)}}" class="lazy post-thumb" alt="{{$mov_view->movie->title}}" title="{{$mov_view->movie->title}}" />
